...
namespace EFCoreHotel_RazorPages.Services.EFService
{
    public class EFBookingService:IBookingService
    {
        ...
        public IEnumerable<Booking> FilterBookingsByDate(DateTime? date=null )
        {
            return context.Bookings.Where(b => b.DateFrom<= @date && b.DateTo >= @date);
        }
        ...
        {
            return context.Bookings.Where(b=>b.HotelNo==hid && b.RoomNo==rid);          
        }
    }
}
